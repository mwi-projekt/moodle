...
if ($existing_la) {
    ...
    $existing_content = $DB->get_record('dhbwio_la_contents', ['la_id' => $existing_la->id], 'id', IGNORE_MISSING);
    if ($existing_content) {
        // Bestehendes LA aktualisieren.
        dhbwio_update_la_content_and_modules($existing_content->id, $content, $modules);
    } else {
        // Sollte nicht vorkommen, aber als Fallback ein neues LA erstellen.
        dhbwio_create_full_la($USER->id, $content, $modules, $app_entryid);
    }
} else {
    // Wenn kein LA existiert, ein neues erstellen (App-Entry-ID mit übergeben).
    dhbwio_create_full_la($USER->id, $content, $modules, $app_entryid);
}
